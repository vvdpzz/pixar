class App.Views.New extends Backbone.View
  ...
  save: (e) ->
    ...
    return false unless @checkPost(@$('#title').val(),@$('#new_question').find('.nicEdit-main').text().length,checked_credit,customized_reputation)
    @model.set({
      title: @$("#title").val(),
  ...
  checkPost:(title,contentLen,otherCredit,otherReputation)->
    unless questionBool
      $("#credit_Tips").hide()
